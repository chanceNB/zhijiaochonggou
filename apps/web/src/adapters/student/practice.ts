...
import { displayKnowledgePoint, displayLearningStateStatus, displayMisconception, displayQuestionSummary, displayTransferValidation } from '@/adapters/student/presentation'
...
export interface PracticeOutcomeVm extends PracticeOutcomeDto {
  ...
  learningStateStatusLabel: string
  transferValidationLabel: string
}
...
export function toPracticeOutcomeVm(dto: PracticeOutcomeDto): PracticeOutcomeVm {
  return { ...dto, accuracyPercent: Math.round(dto.accuracy * 100), learningStateStatusLabel: displayLearningStateStatus(dto.learningStateStatus), transferValidationLabel: displayTransferValidation(dto.transferValidation) }
}
...
export function toWrongBookItemVm(dto: WrongBookItemDto): WrongBookItemVm {
  return {
    ...dto,
    statusLabel: statusLabels[dto.status] ?? '待复习',
    reasonLabel: dto.reasonDisplayName?.trim() || displayMisconception(dto.reason),
    knowledgePointLabel: displayKnowledgePoint(dto.knowledgePointId, dto.knowledgePointName),
    questionSummary: displayQuestionSummary(dto.questionId, dto.questionSummary ?? dto.questionStem),
  }
}
