      ...
      <section class="question-layout">
        ...
        <main class="question-card">
          ...
          <footer class="question-actions">
            <template v-if="feedback?.canAddWrongBook">
              <span v-if="wrongBookAddedForCurrent" class="action-success">✓ 已加入错题本</span>
              <button v-else type="button" class="secondary-action" :disabled="practice.addingWrongBook" @click="addWrongBook">{{ practice.addingWrongBook ? '正在加入...' : '加入错题本' }}</button>
              <button v-if="wrongBookAddedForCurrent" type="button" class="secondary-action" @click="viewWrongBook">查看错题本</button>
            </template>
            <template v-if="canGenerateSimilar">
              <button type="button" class="secondary-action" :disabled="similarBusy" @click="generateSimilar">{{ similarBusy ? 'AI 正在生成...' : '生成类似题' }}</button>
              <span v-if="similarNotice" class="action-success">{{ similarNotice }}</span>
              <span v-else-if="coach.similarError" class="action-error">{{ coach.similarError }}</span>
            </template>
...
const route = useRoute(); const router = useRouter(); const practice = usePracticeStore(); const coach = useCoachStore(); const wrongBook = useWrongBookStore()
const selectedAnswer = ref(''); const submitting = ref(false); const similarBusy = ref(false); const similarNotice = ref('')
...
const wrongBookAddedForCurrent = computed(() => Boolean(feedback.value && practice.addedWrongBookItem?.sourceAttemptId === feedback.value.attemptId))
...
async function addWrongBook() {
  if (!feedback.value) return
  const item = await practice.addWrongBook(feedback.value.attemptId)
  if (item) wrongBook.focusItem(item.wrongItemId)
}
function viewWrongBook() {
  if (practice.addedWrongBookItem) wrongBook.focusItem(practice.addedWrongBookItem.wrongItemId)
  void router.push('/student/wrong-book')
}
...
async function generateSimilar() {
  ...
  if (id) {
    similarNotice.value = '已生成 1 道针对当前错误的类似题，正在打开...'
    await router.push(`/student/practice/${encodeURIComponent(id)}`)
  }
}
function openCoach() {
...
watch(() => practice.currentQuestion?.questionId, () => { selectedAnswer.value = feedback.value?.selectedAnswer ?? ''; similarNotice.value = '' })
watch(practiceSetId, async (nextPracticeSetId, previousPracticeSetId) => {
  if (!nextPracticeSetId || nextPracticeSetId === previousPracticeSetId) return
  await load()
  if (practice.data?.coachSessionId) await coach.restoreSession(practice.data.coachSessionId)
})
onMounted(async () => {
  await load()
...
.action-success,.action-error { align-self: center; font-size: 12px; font-weight: 700; }.action-success { color: #258052; }.action-error { color: #b42318; }
