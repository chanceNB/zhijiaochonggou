  ...
  <section class="runner-page" data-testid="student-practice-runner">
    ...
    <template v-else>
      <header class="runner-header">
        <div><span class="eyebrow">{{ sourceLabel }}</span><h1>定向练习</h1><p>知识点 {{ practice.currentQuestion.knowledgePointId }}</p></div>
      ...
      <section class="question-layout">
        ...
        <main class="question-card">
          ...
          <section v-if="feedback" class="feedback" :class="{ 'feedback--correct': feedback.correct }" data-testid="practice-feedback">
            <div class="feedback__title"><strong>{{ feedback.correct ? '回答正确' : '需要再想一步' }}</strong><span v-if="feedback.misconceptionCode">{{ feedback.misconceptionCode }}</span></div>
