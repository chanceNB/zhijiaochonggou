  ...
  <section class="result-page" data-testid="student-practice-result">
    ...
    <template v-else-if="practice.outcome">
      ...
      <section class="result-summary" aria-label="练习结果摘要">
        ...
        <article><span>学习状态</span><strong class="status-value">{{ practice.outcome.learningStateStatus }}</strong><p>后端返回的状态</p></article>
        <article><span>迁移验证</span><strong class="status-value">{{ practice.outcome.transferValidation || '未返回' }}</strong><p>仅在任务有验证时展示</p></article>
